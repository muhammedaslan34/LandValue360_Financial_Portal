#!/usr/bin/env python3
"""Validate v2.5.0 policy versioning and negotiation economics across project types.
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("release_artifacts/policy-negotiation-scenarios-v2.5.0.json"))
    ...
    with TestClient(app) as client:
        register = client.post("/api/auth/register", json={
            "email": "scenario-owner-v250@example.com",
        ...
        with session_scope() as db:
            ...
            create_staff_user(
                db, email="scenario-admin-v250@example.com", password=VALIDATION_CREDENTIAL,
                full_name="Scenario Policy Admin", role_code="PLATFORM_ADMIN",
            )

        client.post("/api/auth/logout", headers={"X-CSRF-Token": user_csrf})
        admin_login = client.post("/api/auth/login", json={"email": "scenario-admin-v250@example.com", "password": VALIDATION_CREDENTIAL})
        ...
        def publish_policy(name_ar: str, name_en: str, changes: dict[str, Any]) -> dict[str, Any]:
            ...
            response = client.post("/api/admin/financial-policy/versions", json={
                "controls": controls,
                "change_reason": f"v2.5.0 scenario validation - {name_en}",
        ...
        financed = client.post("/api/admin/financial-policy/versions", json={
            ...
            "change_reason": "v2.5.0 financed scenario validation",
            "source_version_id": current["id"],
            "activate": False,
        }, headers={"X-CSRF-Token": admin_csrf})
        ...
        user_login = client.post("/api/auth/login", json={"email": "scenario-owner-v250@example.com", "password": VALIDATION_CREDENTIAL})
    ...
    result = {
        "release": "LandValue360 Financial Portal v2.5.0",
