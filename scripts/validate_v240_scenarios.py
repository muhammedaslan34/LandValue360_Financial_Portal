#!/usr/bin/env python3
"""Audit the v2.4.0 multi-project contract scenario matrix.
...
def main() -> int:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("release_artifacts/contract-scenario-tests-v2.4.0.json"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("release_artifacts/contract-scenario-audit-v2.4.0.json"),
    )
    ...
    check("PORTAL_VERSION", source.get("portal_version") == "2.4.0", "Unexpected portal version.", source.get("portal_version"))
    check("ADAPTER_VERSION", source.get("portal_adapter_version") == "2.4.0", "Unexpected adapter version.", source.get("portal_adapter_version"))
