...
RUN pip install --no-cache-dir --no-deps . && mkdir -p /app/data/private && chown -R lv360:lv360 /app && chmod +x /app/scripts/docker_entrypoint.sh /app/scripts/worker_entrypoint.sh /app/scripts/healthcheck.py
USER lv360
EXPOSE 8090
